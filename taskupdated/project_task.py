...
class task_project(osv.Model): #declaring/creating new class
    ...
    'valid' : fields.boolean("Validation"),
    ...
    'tags' : fields.char('Worksheet link',invisible=True ),
    'working_time_progress' : fields.char('Worksheet link',invisible=True ),
    ...
    #     print "iiiiiiiiiiiiiiii"
    #     rec = self.env['res.users'].search([('id','=',self._uid)])
    #     if rec.task_custom_admin:
...
class task_project_matrials(osv.Model):
    ...
    @api.multi
    def unlink(self):
        if self.task_id.valid == True:
            raise osv.except_osv(_('Error!'),_("'Cannot Delete Record'"))

        return super(task_project_matrials,self).unlink()
...
class employee_form(osv.Model):
    _inherit = 'hr.employee'

    _columns={
    'religion' :fields.char(string='Religion'),
    'blood_group' :fields.char(string='Blood Group'),
    'valid_emp' : fields.boolean("Validation"),
    }

class struct_user_extend(osv.Model):
    _inherit  = 'res.users'
    _columns={
    'task_custom_admin' :fields.boolean(string='Task Admin'),
    }
