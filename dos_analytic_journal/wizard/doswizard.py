...
class validate_account_move_lines(osv.osv_memory):
    ...
    def dos_recalculate_overtime(self, cr, uid, ids, context=None):
        ...
        for line in data_line:
            ...
            print "Code **************************"
        return {'type': 'ir.actions.act_window_close'}
    def update_entries(self, cr, uid, ids, context=None):
        obj_move_line = self.pool.get('account.analytic.line')
        move_ids = []
        if context is None:
            context = {}
        ...
        data_line = obj_move_line.browse(cr, uid, context['active_ids'], context)
        for line in data_line:
            if line.journal_id.code == "TS":
                line.total_no_of_hours = line.unit_amount
            elif line.journal_id.code == "SALMAT":
                line.total_no_of_hours = line.task_mat_id.hours
                line.hours_materials = line.task_mat_id.hours
            else:
                print "Nothing"
        return {'type': 'ir.actions.act_window_close'}
